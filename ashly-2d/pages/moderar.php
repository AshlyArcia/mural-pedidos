...
if(isset($_POST['atualiza'])){
    ...
    $sql = "UPDATE tab SET nome='$nome', email='$email', mensagem='$msg' WHERE id=$idatualiza";
...
if(isset($_GET['acao']) && $_GET['acao'] == 'excluir'){
    ...
    mysqli_query($conexao, "DELETE FROM tab WHERE id=$id") or die("Erro ao deletar: " . mysqli_error($conexao));
    header("Location: moderar.php");
    exit;
}
...
if($editar_id){
    ...
    $res = mysqli_query($conexao, "SELECT * FROM tab WHERE id=$editar_id");
...
$seleciona = mysqli_query($conexao, "SELECT * FROM tab ORDER BY id DESC");
